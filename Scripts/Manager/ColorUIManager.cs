...
public class ColorUIManager : GameSingleton<ColorUIManager>/*MonoBehaviour*/
{
    ...
    public void ShowColor(int index, bool needUpdate)
    {
        if (needUpdate)
        {
            PiUI paletteMenu = GetPiUIOf(Consts.PaletteName);
            paletteMenu.piData[index].hidden = false;
            RegeneratePiMenu(Consts.PaletteName);
        }
        StartCoroutine(ShowHint());
        //if (needUpdate)
            //ChangeMenuState(Consts.PaletteName, new Vector2(Screen.width / 2f, Screen.height / 2f));
    }

    IEnumerator ShowHint()
    {
        PiUI paletteMenu = GetPiUIOf(Consts.PaletteName);
        paletteMenu.hintText.text = "合成新颜色！";
        yield return new WaitForSeconds(1.5f);
        paletteMenu.hintText.text = "";
    }
